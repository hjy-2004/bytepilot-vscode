...
### Added
- **Auto-update detection**: Desktop app now checks for updates on startup via GitHub Releases using `tauri-plugin-updater`. Update manifest (`latest.json`) is auto-generated by CI during release.
- **macOS config file picker**: `cmd_pick_file` now opens at the home directory and includes an "All Files" filter, making it easier to find hidden Claude Code config files.
- **Provider switch key loading**: When switching providers in the desktop app, the API key for the new provider is now loaded from disk automatically, eliminating the need to restart.

### Fixed
- **"Error: undefined" on tool calls**: Tauri `invoke()` throws plain strings (not `Error` objects) when Rust commands return `Err(String)`. Fixed 12 files to use `err?.message || err` instead of `err.message`, so real error messages (e.g. "Access denied: path outside workspace") are shown.
- **New file creation blocked by diff logic**: `write_file` in the desktop adapter treated "file not found" as fatal during diff generation. Now correctly ignores missing original files for `write_file` (new files) while still requiring them for `edit_file`.
- **Session delete state sync**: Deleting the active session now immediately clears the chat UI locally and sends `chat.clear` to the backend. The desktop adapter now switches to the first remaining session instead of hardcoded "default".
- **Input focus lost after send**: Textarea now auto-focuses after sending a message and when streaming completes, eliminating the need to manually re-click the input box.
- **Tool card expansion lag**: Three-fold fix — (1) `setToolRunning`/`updateToolResult` now only creates new references for the specific message containing the target tool call, not all messages; (2) `JSON.stringify(args)` and result truncation memoized with `useMemo`; (3) `preprocessContent()` in MessageBubble memoized.
- **Config save race condition**: `saveCfg()` now debounced at 100ms to prevent rapid `config.set` + `config.setKey` calls from overwriting each other.

### Changed
- **`cmd_pick_file` (Rust)**: Added "All Files" filter and home directory as starting folder.
- **`capabilities/default.json`**: New permissions file for Tauri v2 updater and dialog plugins.
